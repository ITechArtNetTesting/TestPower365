function Invoke-Test39544{
    param( 
 	...
 	Begin
	 {
		...
		$TargetMessage = Invoke-P365FindMessage -TargetMailbox -MessageId $EntryIdVal -FolderPath ("\Inbox\" + $FolderName)
		$TestResults = "" | Select TestCase,Description,TestLastRun,TestResult,Data,ValidationLastRun,ValidationResult,OverAllResult
		$TestResults.TestCase = "39544"
		$TestResults.Description = "Rollback folder remove test"
		$TestResults.TestLastRun = (Get-Date)
		if($TargetMessage -ne $Null){
			$TestResults.TestResult = "Succeeded"
			$TestResults.Data = "\Inbox\" + $FolderName
		}
		else{
			$TestResults.TestResult = "Failed"			
		}
		if($Script:TestResults.ContainsKey("Test39544")){
			$Script:TestResults["Test39544"] = $TestResults
		}
		else{
			$Script:TestResults.Add("Test39544",$TestResults)
		}
