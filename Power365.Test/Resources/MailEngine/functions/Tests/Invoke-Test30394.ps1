function Invoke-Test30394 {
    ...
    Begin {
        ...
        $FolderName = "Test30394-" + (Get-Date).ToString("s")
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            if($Message -ne $null){
                $Body = New-Object Microsoft.Exchange.WebServices.Data.MessageBody  
                $Body.BodyType = [Microsoft.Exchange.WebServices.Data.BodyType]::HTML  
                $Body.Text = "Reply" 
                $PReply = $Post.CreatePostReply();                
                $PReply.Body = $Body
                $PReply.Save();
                $ivItemView = New-Object Microsoft.Exchange.WebServices.Data.ItemView(1)                
                $fiItems = $NewFolder2.FindItems($ivItemView)
                $fiItems.Items[0].Load($psPropertySet)
		        $EntryIdVal = $null		
                [Void]$fiItems.Items[0].TryGetProperty($PR_ENTRYID,[ref]$EntryIdVal)  
                $data.MessageId = $EntryIdVal                
            }
        ...
        if ($Script:TestResults.ContainsKey("Test30394")) {
            $Script:TestResults["Test30394"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test30394", $TestResults)
        }
