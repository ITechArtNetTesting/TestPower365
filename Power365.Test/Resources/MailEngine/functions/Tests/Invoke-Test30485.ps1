function Invoke-Test30485 {
    ...
    Begin {
        ...
        $FolderName = "Test30485-" + (Get-Date).ToString("s")
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $Task.Load($psPropertySet)
            $Task.Delete([Microsoft.Exchange.WebServices.Data.DeleteMode]::HardDelete)
        }  
        else {  
        ...
        if ($Script:TestResults.ContainsKey("Test30485")) {
            $Script:TestResults["Test30485"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test30485", $TestResults)
        }
