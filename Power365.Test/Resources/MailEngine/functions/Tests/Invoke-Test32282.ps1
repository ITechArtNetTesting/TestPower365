function Invoke-Test32282 {
    ...
    Begin {
        ...
        $FolderName = "Test32282-" + (Get-Date).ToString("s")
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $FolderName1 = "Test32282-" + (Get-Date).ToString("s")
            ...
            $FolderName2 = "Test322822-" + (Get-Date).ToString("s")
        ...
        foreach ($ItemId in $Data.ItemIds) {
            ...
            if ($fiItems.Items.Count -eq 1) {
                $fiItems.Items[0].Subject = "Change 1234"
                $fiItems.Items[0].Update([Microsoft.Exchange.WebServices.Data.ConflictResolutionMode]::AlwaysOverwrite)
            }
            else {
                $HasNoteFailed = $false;
            }
        ...
        if ($Script:TestResults.ContainsKey("Test32282")) {
            $Script:TestResults["Test32282"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test32282", $TestResults)
        }
