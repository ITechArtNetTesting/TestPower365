function Invoke-Test34524 {
    ...
    Begin {
        ...
        $NewFolder = new-object Microsoft.Exchange.WebServices.Data.TasksFolder($service)  
        $FolderName = "Test34524-" + (Get-Date).ToString("s")
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $NewFolder1 = new-object Microsoft.Exchange.WebServices.Data.TasksFolder($service)  
            $FolderName1 = "Test34524-Child" + (Get-Date).ToString("s")
       ...
       # $Folder = Get-P365PublicFolderFromPath -TargetMailbox -FolderPath ($RootPath + "\" + $FolderName)
        ...
        if ($Script:TestResults.ContainsKey("Test34524")) {
            $Script:TestResults["Test34524"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test34524", $TestResults)
        }
