function Invoke-Test30518 {
    ...
    Begin {
        ...
        $FolderName = "Test30518-" + (Get-Date).ToString("s")
        ...
        #Do the Search  
        $EntryIdVal = $null
        $data = "" | Select Folder1, Folder2, Folder3, MessageId,Location
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $Appointment.Start = [System.TimeZoneInfo]::ConvertTime((Get-Date),[System.TimeZoneInfo]::Utc)
            $Appointment.End=  [System.TimeZoneInfo]::ConvertTime((Get-Date).AddHours(1),[System.TimeZoneInfo]::Utc)
            $Appointment.EndTimeZone = [System.TimeZoneInfo]::Utc
            $Appointment.StartTimeZone = [System.TimeZoneInfo]::Utc           
        ...
        $TestResults.TestCase = "30518"
        $TestResults.Description = "Appointment Importance Change Check"
        ...
        if ($Script:TestResults.ContainsKey("Test30518")) {
            $Script:TestResults["Test30518"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test30518", $TestResults)
        }
