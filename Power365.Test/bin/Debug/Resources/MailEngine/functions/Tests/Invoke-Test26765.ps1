function Invoke-Test26765{
    param( 
 	...
 	Begin
	 {
		...
		$FolderName = "Test26765-" + (Get-Date).ToString()
		...
		$TestResults.TestCase = "26765"
		$TestResults.Description = "Folder Delete ReCreate"
		$TestResults.TestLastRun = (Get-Date)		
		if($folder -ne $Null){
			$NewFolder.Delete([Microsoft.Exchange.WebServices.Data.DeleteMode]::HardDelete)
			$NewFolder = new-object Microsoft.Exchange.WebServices.Data.Folder($service)  
			$NewFolder.DisplayName = $FolderName
			$NewFolder.FolderClass = "IPF.Contact"
			#Define Folder Veiw Really only want to return one object  
		...
		if($Script:TestResults.ContainsKey("Test26765")){
			$Script:TestResults["Test26765"] = $TestResults
		}
		else{
			$Script:TestResults.Add("Test26765",$TestResults)
		}
