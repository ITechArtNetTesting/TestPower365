function Invoke-Test30471 {
    ...
    Begin {
        ...
        $FolderName = "Test30471-" + (Get-Date).ToString("s")
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $MovedItem = $Email.Move($NewFolder1.Id)
            $MovedItem.Load($psPropertySet)
            $EntryIdVal = $null		
            [Void]$MovedItem.TryGetProperty($PR_ENTRYID, [ref]$EntryIdVal)  
            $data.MessageId = $EntryIdVal
        }  
        else {  
        ...
        if ($Script:TestResults.ContainsKey("Test30471")) {
            $Script:TestResults["Test30471"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test30471", $TestResults)
        }
