function Invoke-Test32347 {
    ...
    Begin {
        ...
        $FolderName = "Test32347-" + (Get-Date).ToString("s")
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $FolderName1 = "Test32347-" + (Get-Date).ToString("s")
            ...
            $NewItem = $Contact.Move($NewFolder1.Id)
            $NewItem.Load($psPropertySet)
            $EntryIdVal = $null		
            [Void]$NewItem.TryGetProperty($PR_ENTRYID,[ref]$EntryIdVal)  
            $data.MessageId = $EntryIdVal
        ...
        if ($Script:TestResults.ContainsKey("Test32347")) {
            $Script:TestResults["Test32347"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test32347", $TestResults)
        }
