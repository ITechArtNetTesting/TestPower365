    ...
    param( 
 	...
 	Begin
	 {
		...
		if($Script:TestResults.ContainsKey("Test30378")){
			$Script:TestResults["Test30378"].ValidationResult = "Failed"	
			$Script:TestResults["Test30378"].OverAllResult = "Failed"	
			$Okay = $false
			$Script:TestResults["Test30378"].ValidationLastRun = (Get-Date)
			$Script:TestResults["Test30378"].Data.Folder2
			$Folder = Get-P365PublicFolderFromPath -TargetMailbox -FolderPath $Script:TestResults["Test30378"].Data.Folder
			$HasNoteFailed = $true
			foreach($ItemId in $Script:TestResults["Test30378"].Data.ItemIds){
				$Okay = $true;
