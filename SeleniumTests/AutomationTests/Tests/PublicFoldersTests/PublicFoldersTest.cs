	...
	public class PublicFoldersTest : LoginAndConfigureTest
	{
		...
		public void Automation_IN_PublicFoldersTest()
		{
		    ...
		    try
		    {
		        ...
		        {
                    Log.Info("Starting SourcePrepScript.ps1");
		            using (
		            ...
		            )
		            {
                        Task sourceStdOut = Task.Run(() =>
                        {
                            ...
                            {
                                var line = sourcePreparation.StandardOutput.ReadLine();
                                Log.Debug(line);
                        ...
                        {
                            while (!sourcePreparation.StandardError.EndOfStream)
                            {
                                var line = sourcePreparation.StandardError.ReadLine();
                                Log.Error(line);
                            }
                        });
		        ...
		        while (!result && counter < 3)
		        {
                    Log.Info("Starting TargetPrepScript.ps1");
		            ...
		            {
                        Task targetStdOut = Task.Run(() =>
                        {
                            while (!targetPreparation.StandardOutput.EndOfStream)
                            {
                                var line = targetPreparation.StandardOutput.ReadLine();
                                Log.Debug(line);
                                if (line.Contains("Public Folder successfully created"))
                                {
                                    result = true;
                        ...
                        Task targetStdError = Task.Run(() =>
                        {
                            while (!targetPreparation.StandardError.EndOfStream)
                            {
                                var line = targetPreparation.StandardError.ReadLine();
                                Log.Error(line);
                            }
                        });

                        Task.WaitAll(targetStdOut, targetStdError);
                ...
                Log.Info("Starting UI Automation");
                ...
                Log.Info("Starting PfAutomationScript.ps1");
		                                                                                                       ...
		                                                                                                       $" -TargetProxyAddress {RunConfigurator.GetValueByXpath("//metaname[text()='client2']/..//metaname[text()='project2']/..//metaname[text()='entry5']/..//targetproxy")}"))
		        {
                    ...
                    Task validateStdError = Task.Run(() =>
                    {
                        while (!mainScript.StandardError.EndOfStream)
                        {
                            var line = mainScript.StandardError.ReadLine();
                            Log.Error(line);
                        }
                    });

                    Task.WaitAll(validateStdOut, validateStdError);
		    ...
		    {
                Log.Error("PublicFolderTests Failed", e);
