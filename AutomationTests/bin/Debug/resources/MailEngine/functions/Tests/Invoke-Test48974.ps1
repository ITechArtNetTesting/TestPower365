function Invoke-Test48974{
    param( 
 	...
 	Begin
	 {
		...
		$FolderName = "Test48974-" + (Get-Date).ToString()
		...
		#Do the Search  
		$EntryIdVal = $null
		$data = "" | Select Folder1,SourceId,FirstTargetPermission,SecondTargetPermission,NewFolderName
		...
		$TestResults.TestCase = "48974"
		$TestResults.Description = "Folder Permission/Rename"
		...
		if($folder -ne $Null){
    		...
    		if($fperm.UserId.PrimarySmtpAddress -ne $null){  
				...
				if($fperm.UserId.PrimarySmtpAddress.ToLower() -eq $FirstTargetPermission.ToLower()){
						...
						$NewFolder.Permissions.Add($newfp) 
						$NewFolder.DisplayName = $NewFolder.DisplayName + "-NewName"
						$NewFolder.Update() 
						$folderid= new-object Microsoft.Exchange.WebServices.Data.FolderId([Microsoft.Exchange.WebServices.Data.WellKnownFolderName]::Drafts,$MailboxName)   
						$draftsFolder = [Microsoft.Exchange.WebServices.Data.Folder]::Bind($service,$folderid)
						$NewFolder.Move($draftsFolder.Id)
		...
		if($Script:TestResults.ContainsKey("Test48974")){
			$Script:TestResults["Test48974"] = $TestResults
		}
		else{
			$Script:TestResults.Add("Test48974",$TestResults)
		}
