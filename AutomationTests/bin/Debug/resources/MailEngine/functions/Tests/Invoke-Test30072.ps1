function Invoke-Test30072 {
    ...
    Begin {
        ...
        if ($findFolderResults.TotalCount -eq 0) {  
            ...
            $FolderName1 = "Test30072-Child" + (Get-Date).ToString("s")
        ...
        $newfp = new-object Microsoft.Exchange.WebServices.Data.FolderPermission($FirstSourcePermission, $PermissiontoAdd)  
        $NewFolder.Permissions.Clear()
        $NewFolder.Permissions.Add($newfp)   	    
        $NewFolder.Update()        
        $TestResults = "" | Select TestCase, Description, TestLastRun, TestResult, Data, ValidationLastRun, ValidationResult, OverAllResult
        $TestResults.TestCase = "30072"
        ...
        if ($Script:TestResults.ContainsKey("Test30072")) {
            $Script:TestResults["Test30072"] = $TestResults
        }
        else {
            $Script:TestResults.Add("Test30072", $TestResults)
        }
