function Invoke-P365FindPublicFolder{
    param( 
 	...
 	Begin
	 {
		...
		if([String]::IsNullOrEmpty($ParentFolderPath)){
			...
			if($Archive.IsPresent){
				$InboxFolder = Get-P365PublicFolderFromPath -FolderPath $ParentFolderPath -TargetMailbox -Archive
			}else{
				$InboxFolder = Get-P365PublicFolderFromPath -FolderPath $ParentFolderPath -TargetMailbox
			}			
