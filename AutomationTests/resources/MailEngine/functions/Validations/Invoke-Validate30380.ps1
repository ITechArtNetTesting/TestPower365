    ...
    param( 
 	...
 	Begin
	 {
		...
		if($Script:TestResults.ContainsKey("Test30380")){
			$Script:TestResults["Test30380"].ValidationResult = "Failed"	
			$Script:TestResults["Test30380"].OverAllResult = "Failed"	
			$Okay = $false
			$Script:TestResults["Test30380"].ValidationLastRun = (Get-Date)
			$Script:TestResults["Test30380"].Data.Folder2
			$Folder = Get-P365PublicFolderFromPath -TargetMailbox -FolderPath $Script:TestResults["Test30380"].Data.Folder
			$HasNoteFailed = $true
			foreach($ItemId in $Script:TestResults["Test30380"].Data.ItemIds){
				$Okay = $true;
			...
			if($HasNoteFailed -band $Okay){
				$Script:TestResults["Test30380"].ValidationResult = "Succeeded"
				$Script:TestResults["Test30380"].OverAllResult = "Successful"
