    ...
    param( 
 	...
 	Begin
	 {
		...
		if($Script:TestResults.ContainsKey("Test30384")){
			$Script:TestResults["Test30384"].ValidationResult = "Failed"	
			$Script:TestResults["Test30384"].OverAllResult = "Failed"	
			$Okay = $false
			$Script:TestResults["Test30384"].ValidationLastRun = (Get-Date)
			$Script:TestResults["Test30384"].Data.Folder2
			$Folder = Get-P365PublicFolderFromPath -TargetMailbox -FolderPath $Script:TestResults["Test30384"].Data.Folder
			$HasNoteFailed = $true
			foreach($ItemId in $Script:TestResults["Test30384"].Data.ItemIds){
				$Okay = $true;
			...
			if($HasNoteFailed -band $Okay){
				$Script:TestResults["Test30384"].ValidationResult = "Succeeded"
				$Script:TestResults["Test30384"].OverAllResult = "Successful"
