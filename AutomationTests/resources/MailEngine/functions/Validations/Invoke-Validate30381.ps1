    ...
    param( 
 	...
 	Begin
	 {
		...
		if($Script:TestResults.ContainsKey("Test30381")){
			$Script:TestResults["Test30381"].ValidationResult = "Failed"	
			$Script:TestResults["Test30381"].OverAllResult = "Failed"	
			$Okay = $false
			$Script:TestResults["Test30381"].ValidationLastRun = (Get-Date)
			$Script:TestResults["Test30381"].Data.Folder2
			$Folder = Get-P365PublicFolderFromPath -TargetMailbox -FolderPath $Script:TestResults["Test30381"].Data.Folder
			$HasNoteFailed = $true
			foreach($ItemId in $Script:TestResults["Test30381"].Data.ItemIds){
				$Okay = $true;
			...
			if($HasNoteFailed -band $Okay){
				$Script:TestResults["Test30381"].ValidationResult = "Succeeded"
				$Script:TestResults["Test30381"].OverAllResult = "Successful"
