    ...
    param( 
 	...
 	Begin
	 {
		...
		if($Script:TestResults.ContainsKey("Test32267")){
			$Script:TestResults["Test32267"].ValidationResult = "Failed"	
			$Script:TestResults["Test32267"].OverAllResult = "Failed"	
			$Okay = $false
			$Script:TestResults["Test32267"].ValidationLastRun = (Get-Date)
			$Script:TestResults["Test32267"].Data.Folder2
			$Folder = Get-P365PublicFolderFromPath -TargetMailbox -FolderPath $Script:TestResults["Test32267"].Data.Folder2
			$HasNoteFailed = $true
			foreach($ItemId in $Script:TestResults["Test32267"].Data.ItemIds){
				$Okay = $true;
			...
			if(($HasNoteFailed) -band $Okay){
					$Script:TestResults["Test32267"].ValidationResult = "Succeeded"
					$Script:TestResults["Test32267"].OverAllResult = "Successful"
			}
			else{
				$Script:TestResults["Test32267"].ValidationResult = "Failed"	
				$Script:TestResults["Test32267"].OverAllResult = "Failed"		
